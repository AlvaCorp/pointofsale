 ...
 * @property string $tipe_kendaraan_id
...
class Kendaraan extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			//array('merk_kendaraan_id, kategori_kendaraan_id, bahan_bakar_id, jenis_kendaraan_id, no_kendaraan, no_mesin', 'required'),
			array('jenis_kendaraan_id', 'required'),
			array('id, kategori_kendaraan_id, bahan_bakar_id, jenis_kendaraan_id, tahun_kendaraan', 'numerical', 'integerOnly'=>true),
			array('no_kendaraan', 'length', 'max'=>16),
            array('tahun_kendaraan', 'length', 'max'=>4),
			array('no_mesin', 'length', 'max'=>128),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, merk_kendaraan_id, kategori_kendaraan_id, bahan_bakar_id, jenis_kendaraan_id, no_kendaraan, no_mesin, tahun_kendaraan, tipe_kendaraan_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'detailKendaraan' => array(self::HAS_MANY, 'DetailKendaraan', 'kendaraan_id'),
                        'tipeKendaraan' => array(self::HAS_MANY, 'TipeKendaraan', 'tipe_kendaraan_id'),
	...
	public function search()
	{
		...
		$criteria->compare('merk_kendaraan_id',$this->merk_kendaraan_id);
                $criteria->compare('tipe_kendaraan_id',$this->tipe_kendaraan_id);
